...
[source,kotlin,]
----
val step1: StepResult<Int> = step { 1+1 } `assert that` { eq(2, it) }

val step2: StepResult<Double> = step { step1() + 1.5 } `assert that` { eq(3.5, it) }

step { step2.invoke().toString() } `assert that` { eq("3.5", it) }
----
...
val step1 = nestedScenario<Double> {
    ...
    val step11: StepResult<Int> = step { 1+1 } `assert that` { eq(2, it) }

    val step12: StepResult<Double> = step { step11() + 1.5 } `assert that` { eq(3.5, it) }

    returns { step12() }
}

step { step1() + 4 } `assert that` { eq(7.5, it) }
----
...
[source,kotlin,]
----
val step1 = step { 1 + 1 } `map result to` { it * 1.0 } `assert that` { eq(2, it) }

step { step1() * 3 } `assert that` { eq(6.0, it) }
----
