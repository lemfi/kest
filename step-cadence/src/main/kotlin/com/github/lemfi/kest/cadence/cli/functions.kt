...
inline fun <reified R> ScenarioBuilder.`given activity call`(
    ...
    return Step(name!!, { ActivityCallExecutionBuilder<R>(R::class.java).apply(h).build() }, retry = retryStep).apply {
        steps.add(this)
    }.postExecution
}
...
inline fun <R> ScenarioBuilder.`given workflow`(
    ...
    return Step(name!!, { WorkflowExecutionBuilder<R>().apply(h).build() }, retry = retryStep).apply {
        steps.add(this)
    }.postExecution
}